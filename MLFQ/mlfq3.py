...
# This is calculation part of fairness.
def calc_fairness(actual, prediction):
    '''
    input:
        actual: array
        prediction: array
    output:
        fairness: float
    '''
    sqerr = (sum([i - j for i, j in zip(actual, prediction)]))**2
    print(sqerr)
    sqdino = len(actual)*sum([(i - j)**2 for i, j in zip(actual, prediction)])
    print(sqdino)
    fairness = sqerr/sqdino
    return fairness
...
while finishedJobs < totalJobs:
    ...
    if currJob == 0:
        priorities_0.append(currQueue)
        priorities_1.append(999)
        priorities_2.append(999)
    elif currJob == 1:
        priorities_0.append(999)
        priorities_1.append(currQueue)
        priorities_2.append(999)
    elif currJob == 2:
        priorities_0.append(999)
        priorities_1.append(999)
        priorities_2.append(currQueue)
...
for i in range(numJobs):
    ...
    actual.append(turnaround)

print('\n  Avg %2d: startTime n/a - response %.2f - turnaround %.2f' % (i,
                                                                        float(responseSum)/numJobs,
                                                                        float(turnaroundSum)/numJobs))
print('\n')
# printgantt(numQueues, currTime, priorities_0, priorities_1, priorities_2)
print(actual)
print(pretime)
fairness = calc_fairness(actual, pretime)
